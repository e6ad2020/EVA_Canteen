...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateCartBadge() {
        ...
        // Also update the preview button indicator if a product is being viewed
        if (addToCartPreviewButton && addToCartPreviewButton.dataset.itemId && typeof updatePreviewIndicator === 'function') {
            updatePreviewIndicator(addToCartPreviewButton.dataset.itemId);
        }
    }
    ...
    function updatePreviewIndicator(id) {
        const indicator = document.getElementById('preview-quantity-indicator');
        if (!indicator) return;
        const totalQty = cart.reduce((sum, item) => item.id === id ? sum + item.quantity : sum, 0);
        indicator.textContent = totalQty;
        if (totalQty > 0) indicator.classList.add('visible');
        else indicator.classList.remove('visible');
    }

    function showItemPreview(id, n = true) {
        const d = getProductData(id);
        if (!d || !previewItemImage || !previewItemName || !previewItemDescription || !previewItemPrice || !addToCartPreviewButton) return;
        const nm = getText(d.name_key),
            ds = getText(d.description_key || 'item_desc_default'),
            p = formatPrice(d.price);
        previewItemImage.src = d.image;
        previewItemImage.alt = nm;
        previewItemName.textContent = nm;
        previewItemDescription.textContent = ds;
        previewItemPrice.textContent = p;
        addToCartPreviewButton.dataset.itemId = id;

        updatePreviewIndicator(id); // Update indicator when opening

        setPreviewButtonState(false);
        if (n) showScreen('screen-7');
    ...
    function populateDiscoveryMode() {
        ...
        categories.filter(category => category.key !== 'archive').forEach(category => {
            ...
            if (productsToDisplay.length > 0) { categorySection.appendChild(categoryGrid); discoveryCategoriesContainer.appendChild(categorySection); }
        }); updateCartBadge();
    }
